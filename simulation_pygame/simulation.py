...
vehiclecounttcoods = [(200,255), (800,155), (1000,555),(400,755)]
...
def repeat():
    global currentGreen, currentYellow, nextGreen, signaltime   
    
    while(signals[currentGreen].green>0):   # while the timer of current green signal is not zero
        if(countveh[currentGreen]<1):
    ...
    signaltime=countveh.copy()
    sig=countveh.copy()
    sig.sort()
    for k in range(0,3):
        #if(sig[k]==countveh[k]):
        signaltime[k]=int(countveh[k]*0.40)
            #aa=signaltime[k]
        
    # reset all signal times of current signal to default/random times
    if(randomGreenSignalTimer): #
        signals[currentGreen].green = 10 #signaltime[k] #10 #random.randint(randomGreenSignalTimerRange[0],randomGreenSignalTimerRange[1])
    else:
        signals[currentGreen].green = 10 #signaltime[k] #10 #defaultGreen[currentGreen]
...
# 'bicycle' 'cars' 'Motorcycles' 'Bus' 'Trucks'
    ...
    while(True):
        ...
        for veh in myallowedveh:
           #####for j in range(0,int(words[myallowedveh.index(veh)+1])%5*4):
            for j in range(0,int(words[myallowedveh.index(veh)+1])%5*2):
                vehicle_type = myallowedveh.index(veh)
                Vehicle(lane_number, vehicleTypes[vehicle_type], direction_number, directionNumbers[direction_number], will_turn)
                countveh[direction_number]+=1
                time.sleep(1)
        #signaltime[o]=int(words[myallowedveh(veh)
...
class Main:
    global allowedVehicleTypesList ,display , aa
    i = 0
    display=0
    ...
    pygame.display.set_caption("Traffic SIMULATION for Analysis")
    ...
    while True:
        ...
        for i in range(0,noOfSignals):  
            signalTexts[i] = font.render(str(signals[i].signalText), True, black, green) #---white, black)
            screen.blit(signalTexts[i],signalTimerCoods[i])
       
        # display vehicle count
        for i in range(0,noOfSignals):
            display = vehicles[directionNumbers[i]]['crossed']
            vehicleCountTexts[i] = font.render(str(display), True, black, white)
            screen.blit(vehicleCountTexts[i],vehicleCountCoods[i])
            vehiclecounttext = font.render(("Vehicle count: "+str(countveh[i])+" -- "+aa), True, black, white)
            screen.blit(vehiclecounttext ,vehiclecounttcoods[i]) 
